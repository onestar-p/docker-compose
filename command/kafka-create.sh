...
show_help() {
    ...
    echo "命令列表:"
    echo ""
    echo "  ${GREEN}topic${NC} <name> [options]    创建 Topic"
    echo "    参数:"
    echo "      -p, --partitions <num>        分区数（默认: 3）"
    echo "      -r, --replication <num>       副本因子（默认: 1）"
    echo "      -c, --config <key=value>      额外配置"
    echo ""
    echo "  ${GREEN}group${NC} <topic> <group-name>  测试 Consumer Group（启动消费者）"
    echo ""
    echo "  ${GREEN}delete-topic${NC} <name>         删除 Topic"
    echo ""
    echo "  ${GREEN}delete-group${NC} <name>         删除 Consumer Group"
    echo ""
    echo "示例:"
    echo "  # 创建默认配置的 topic"
